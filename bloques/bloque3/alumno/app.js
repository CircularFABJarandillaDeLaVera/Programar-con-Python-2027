(() => {
  ...
  function ficha() {
    ...
    main.innerHTML = `
      <section class="panel cf-panel">
        ...
        <h1>${esc(data.bloque || 'Funciones y programacion modular')}</h1>
  ...
  function flash() {
    ...
    const cards = Array.isArray(data.flashcards) && data.flashcards.length
      ? data.flashcards
      : [['Datos no cargados', 'Revisa la carga de data/bloque3.js.']];
    main.innerHTML = `
  ...
  function casos() {
    ...
    const cases = Array.isArray(data.casos) && data.casos.length
      ? data.casos
      : [{ titulo: 'Datos no cargados', situacion: 'No se han cargado casos para esta vista.', preguntas: ['Revisa la carga de data/bloque3.js.'] }];
  ...
  function test() {
    ...
    const questions = Array.isArray(data.test) && data.test.length
      ? data.test
      : [{ pregunta: 'No se han cargado microevaluaciones.', opciones: ['Revisar datos', 'Continuar'], correcta: 0, explicacion: 'Revisa la carga de data/bloque3.js.' }];
    const selected = {};
    main.innerHTML = `
