(() => {
  ...
  const slides = Array.isArray(data.slides) && data.slides.length
    ? data.slides
    : [{ titulo: data.bloque || 'Funciones y programacion modular', objetivo: 'Datos del bloque no cargados.' }];
  ...
  function render(index) {
    ...
    `;
    note.innerHTML = `<strong>Nota docente:</strong> ${esc(slide.nota || slide.error_util || 'Datos de acompanamiento docente no cargados.')}`;
  }
