(() => {
  ...
  function ficha() {
    ...
    main.innerHTML = `
      <section class="panel cf-panel">
        ...
        <h1>${esc(data.bloque || 'Fundamentos y logica')}</h1>
  ...
  function flash() {
    ...
    const cards = Array.isArray(data.flashcards) && data.flashcards.length
      ? data.flashcards
      : [['Datos no cargados', 'Contenido no cargado.']];
    main.innerHTML = `
  ...
  function casos() {
    ...
    const cases = Array.isArray(data.casos) && data.casos.length
      ? data.casos
      : [{ titulo: 'Datos no cargados', situacion: 'Casos no cargados.', preguntas: ['Revisa que los datos del bloque esten disponibles.'] }];
  ...
  function test() {
    ...
    const questions = Array.isArray(data.test) && data.test.length
      ? data.test
      : [{ pregunta: 'Microevaluaciones no cargadas.', opciones: ['Datos no cargados'], correcta: 0, explicacion: 'Revisa que los datos del bloque esten disponibles.' }];
    const selected = {};
    main.innerHTML = `
