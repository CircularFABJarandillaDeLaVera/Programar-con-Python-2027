(() => {
  ...
  function ficha() {
    ...
    main.innerHTML = `
      <section class="panel cf-panel">
        ...
        <h1>${esc(data.bloque || '{{BLOQUE_TITULO}}')}</h1>
        <p>${esc(data.descripcion || 'Resumen de trabajo pendiente de generar.')}</p>
      </section>
      <section class="panel cf-panel columns">
        <div><h2>Objetivos</h2><p>Completar con objetivos validados del bloque.</p></div>
        <div><h2>Ideas clave</h2><p>Completar con los conceptos principales.</p></div>
      </section>
    `;
  }
  ...
  function flash() {
    ...
    const cards = Array.isArray(data.flashcards) && data.flashcards.length
      ? data.flashcards
      : [['Pregunta placeholder', 'Respuesta placeholder']];
    main.innerHTML = `
  ...
  function casos() {
    ...
    const cases = Array.isArray(data.casos) && data.casos.length
      ? data.casos
      : [{ titulo: 'Caso placeholder', situacion: 'Situacion pendiente.', preguntas: ['Pregunta de analisis pendiente.'] }];
  ...
  function test() {
    ...
    const questions = Array.isArray(data.test) && data.test.length
      ? data.test
      : [{ pregunta: 'Pregunta placeholder', opciones: ['Opcion A', 'Opcion B'], correcta: 0 }];
    const selected = {};
    main.innerHTML = `
